"""
Improved Graph WaveNet Implementation
All paper features implemented correctly with optimizations
"""
...
class GatedTCN(nn.Module):
    ...
    IMPROVED: Better skip connection handling
    """
...
class GraphWaveNet(nn.Module):
    ...
    def set_static_adj(self, adj):
        ...
        FIXED: Add self-loops BEFORE normalization (already correct in your code)
        """
        device = adj.device
        orig_id = id(adj)
    ...
    def forward(self, x, adj=None):
        ...
        for layer in self.st_layers:
            ...
            # IMPROVED: Pool temporal dimension for each skip
...
def masked_mae_loss(pred, target, null_val=0.0):
    """
    IMPROVED: Handles both NaN and null_val masking
    """
...
def masked_mape_loss(pred, target, null_val=0.0, epsilon=1e-5):
    """
    IMPROVED: Masked MAPE with numerical stability
    """
    mask = ~torch.isnan(target)
    mask = mask & (target != null_val)
    mask = mask.float()
...
if __name__ == "__main__":
    print("="*60)
    print("IMPROVED GRAPH WAVENET - VALIDATION TEST")
    print("="*60)
    ...
    print("="*60)
    print("✓ Layer Normalization after GCN (Paper Section 4.2)")
    print("✓ Better skip connection temporal handling")
    print("✓ Improved loss functions with masking")
    print("✓ Proper self-loop handling (already correct)")
    print("✓ Numerical stability in MAPE")
    print("="*60)
